...
- **デザイン×実装の越境**：デザイナー不在期の Figma での UI デザイン制作、After Effects で自作したアニメーションの Lottie 組み込み、テーマカラー（着せ替え）の配色設計と切替機構の実装
- **アーキテクチャ**：SPM マルチモジュール構成の Clean Architecture への段階的移行、RxSwift → Combine 移行、パフォーマンス改善
- **一気通貫**：API 仕様の調整 → ドメイン設計 → UI 実装 → 段階リリース → App Store 審査対応まで

## 🤖 AI-Driven Development

- AI コーディングエージェント（Claude Code）を日常の実装フローに組み込み、大規模機能の設計・実装・リファクタリングに活用
- デザイントークンの誤用を書き込み前にブロックし、固定色・system フォント等の違反パターンを自動検知するフックを開発してチームに展開
- Figma デザインからの実装を、ツール出力の検算手順まで含めて再現性のあるエージェント用スキルとして自作
- コーディング規約・アーキテクチャルールをエージェントが参照できる形に体系化し、生成コードの品質を仕組みで担保
...
![Combine](https://img.shields.io/badge/Combine-8E44AD?style=flat&logo=apple&logoColor=white)

### Apple Frameworks
![CoreLocation](https://img.shields.io/badge/CoreLocation-000000?style=flat&logo=apple&logoColor=white)
